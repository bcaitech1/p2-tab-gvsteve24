...
def add_tier(train, test):
    tiers = [chr(65+i) for i in range(5)]
    sum_of_cus_train = train[['customer_id', 'total-sum']]
    sum_of_cus_test = test[['customer_id', 'total-sum']]
    tier_bin_train = pd.qcut(x=sum_of_cus_train['total-sum'], q=5, labels=tiers)
    tier_bin_test = pd.qcut(x=sum_of_cus_test['total-sum'], q=5, labels=tiers) 
    tier_bin_train = tier_bin_train.to_frame(name="tier")
    tier_bin_test = tier_bin_test.to_frame(name="tier")
    tier_bin_train['customer_id'] = sum_of_cus_train['customer_id']
    tier_bin_test['customer_id'] = sum_of_cus_test['customer_id']
    train_out = train.merge(tier_bin_train, on=['customer_id'])
    test_out = test.merge(tier_bin_test, on=['customer_id'])
...
def feature_engineering1(df, year_month):
    ...
    all_train_data, test_data = add_tier(all_train_data, test_data)
